...
const Tab = props => {
  const {details, onSelectType, type} = props
  const {tabId, displayText} = details

  const selectedStyling = tabId === type ? 'highlight' : ''
  ...
  return (
    <li className="tab-item">
      <button
        type="button"
        className={`tab-button ${selectedStyling}`}
        onClick={selecteType}
      >
        {displayText}
      </button>
...
class MatchGame extends Component {
  ...
  onTapImage = id => {
    ...
    const areTheySame = mainImageId === clickedImage
    if (areTheySame === true) {
      const i = Math.floor(Math.random() * numberOfImages)
      this.setState(prevState => ({
        score: prevState.score + 1,
        firstImage: imagesList[i],
      }))
    } else {
      this.setState({isGameOn: false})
      clearInterval(this.timerId)
  ...
  onRestartGame = () => {
    this.setState({score: 0, seconds: 60, isGameOn: true})
    this.timerStarts()
  }

  renderGamePart = () => {
    const {thumbnails, type, firstImage} = this.state
    const filteredList = thumbnails.filter(each => each.category === type)
    const imgUrl = firstImage.imageUrl
    return (
  ...
  renderScoreCard = () => {
    ...
    return (
      <div className="score-card-container">
        ...
        <h1 className="score-marks">{score} </h1>
        <button
          type="button"
  ...
  render() {
    ...
    return (
      <div className="container">
        ...
        {isGameOn && this.renderGamePart()}
        {!isGameOn && this.renderScoreCard()}
      </div>
    )
  }
}
